...
import scottie.cs301.Imports.GameFramework.actionMsg.ReadyAction;
import scottie.cs301.Imports.GameFramework.actionMsg.TimerAction;
import scottie.cs301.Imports.GameFramework.infoMsg.BindGameInfo;
import scottie.cs301.Imports.GameFramework.infoMsg.GameOverInfo;
...
public abstract class LocalGame implements Game, Tickable {
	...
	private void receiveMessage(Message msg) {
		if (msg.obj instanceof GameAction) { // ignore if not GameAction
			...
			if (action instanceof MyNameIsAction &&
				...
				Log.i("LocalGameActual", "received 'myNameIs' ("+mnis.getName()+")");
				...
				if (playerNameCount >= playerNames.length) {
					Log.i("LocalGameActual", "broadcasting player names");
				...
				Log.i("LocalGameActual", "got 'ready' ("+playerNames[playerIdx]+")");
				...
				if (playerReadyCount >= playerNames.length) {
					...
					Log.i("LocalGameActual", "broadcasting initial state");
	 ...
	 */
	public abstract boolean makeMove(GameAction action);
